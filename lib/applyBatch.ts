...
export async function applyBatch(batchId: string) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const bulkVariantOps: mongoose.AnyBulkWriteOperation[] = [];
      const ledgerDocs: object[] = [];
      ...
      for (const [variantId, qty] of Array.from(byVariant.entries())) {
        ...
        bulkVariantOps.push({
          updateOne: {
            filter: { _id: v._id },
            update: { $set: { [`stock.${store}`]: newQty } },
          },
        });
        ledgerDocs.push({ variant: v._id, store, delta, reason: ledgerReason, batch: batch._id, balanceAfter: newQty });
        unitsProcessed += qty;
      }

      if (bulkVariantOps.length > 0) {
        await Variant.bulkWrite(bulkVariantOps, { session });
      }
  ...
  try {
    await session.withTransaction(async () => {
      ...
      for (const [variantId, qty] of Array.from(byVariant.entries())) {
        ...
        ledgerDocs.push({ variant: v._id, store, delta, reason: "reversal", batch: batch._id, balanceAfter: newQty });
      }
