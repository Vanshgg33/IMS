...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.productName !== undefined) variant.productName = body.productName.trim();
    if (body.variantLabel !== undefined) variant.variantLabel = body.variantLabel.trim();
    ...
    if (body.productName !== undefined || body.variantLabel !== undefined) {
      ...
      const newNameKey = normalizeName(variant.nameCanonical);
      if (newNameKey !== variant.nameKey) {
        const collision = await Variant.findOne({ nameKey: newNameKey, _id: { $ne: variant._id } });
        if (collision) {
          return NextResponse.json({ error: "Another variant with this name already exists" }, { status: 409 });
        }
      }
      variant.nameKey = newNameKey;
    }
