...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const matched = batch.rows.filter((r: any) => r.status === "matched").length;
  const unmatched = batch.rows.filter((r: any) => r.status === "unmatched").length;
  batch.totals.matched = matched;
  batch.totals.unmatched = unmatched;
